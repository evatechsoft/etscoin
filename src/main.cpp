...
#if defined(NDEBUG)
# error "Etscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x07657c93616aebe1277005161c4f9f5152a9bbbd4bde03b4a3d640105d795ccf");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Etscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Etscoin Signed Message:\n";
...
{
    // Etscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Etscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.35 * 24 * 60 * 60; // Etscoin: 3.5 days
static const int64 nTargetSpacing = 5; // Etscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Etscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Etscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x07657c93616aebe1277005161c4f9f5152a9bbbd4bde03b4a3d640105d795ccf");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1416522403;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 582177;

        if (fTestNet)
        {
            block.nTime    = 1416521006;
        ...
        assert(block.hashMerkleRoot == uint256("0x52b2455696516c14fde6fb8bf13891f34a49b339b99aa863942e5cd54db71fc1"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Etscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EtscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EtscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EtscoinMiner(CWallet *pwallet)
{
    printf("EtscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("etscoin-miner");
    ...
    try { loop {
        ...
        printf("Running EtscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EtscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EtscoinMiner, pwallet));
}
